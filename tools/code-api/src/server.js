...
const HAS_BWRAP = fssync.existsSync(BWRAP_PATH);
...
const buildSandboxSpawn = (cmd, cwd) => {
...
const spawnSandbox = ({ cmd, cwd }) =>
  new Promise((resolve) => {
    const sandbox = buildSandboxSpawn(cmd, cwd);
    const child = spawn(sandbox.bin, sandbox.args, {
      cwd,
      detached: true,
...
app.post('/exec', async (req, res) => {
  ...
  if (!HAS_BWRAP) {
    return res.status(503).json({
      error: 'Service unavailable',
      details: 'bubblewrap is required for sandbox isolation but is not installed',
    });
  }
