...
- **Process isolation**: each `/exec` call runs in a separate process group with hard timeout and `ulimit` controls.
- **Filesystem sandbox**: code is executed with `bubblewrap` in a restricted mount namespace.
- **Directory isolation**: each session workspace is mounted as `/mnt/data` (matching tool expectations).
- **Attribution model**:
...
- `bubblewrap` is required at runtime; `/exec` returns `503` if it is unavailable.
